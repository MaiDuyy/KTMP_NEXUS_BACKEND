// services/chat-service/src/services/thread.service.ts
// Thread System for MSG-07: Thread replies
...
export class ThreadService {
  ...
  async createThreadReply(
    ...
    if (parentMessage.destroy) {
      throw new Error('Cannot reply to a deleted message!');
    }

    // Determine rootThreadId (for nested threads, use parent's root or parent itself)
    ...
    // Update parent's reply count (atomic increment)
    await prisma.message.update({
      where: { id: parentId },
      data: { replyCount: { increment: 1 } },
    });

    // Publish event for realtime broadcast
    await publishEvent(EventSubjects.THREAD_REPLY_CREATED || 'thread.reply.created', {
      id: reply.id,
      chatId: reply.chatId,
      parentId: reply.parentId,
      rootThreadId: reply.rootThreadId,
  ...
  async getThreadReplies(
    ...
    const parentMessage = await prisma.message.findUnique({
      where: { id: parentId },
      select: { id: true, chatId: true, replyCount: true },
    });
    ...
    if (cursor) {
      whereCondition.time = { gt: new Date(cursor) };
    }
    ...
    const replies = await prisma.message.findMany({
      ...
      orderBy: { time: 'asc' }, // Oldest first for threads
      take,
    });
  ...
  /**
   * Get thread preview (latest 3 replies + count)
   */
  async getThreadPreview(parentId: string) {
    ...
    return {
      ...
      latestReplies: latestReplies.reverse(), // Chronological order
    };
  }

  /**
   * Get threads with recent activity in a chat
   */
  async getActiveThreads(chatId: string, limit = 10) {
    // Find parent messages that have replies
    const threadsWithReplies = await prisma.message.findMany({
      where: {
        chatId,
        replyCount: { gt: 0 },
        destroy: false,
  ...
  /**
   * Get participants who replied in a thread
   */
  ...
  /**
   * Group reactions by emoji (same as in message.service)
   */
