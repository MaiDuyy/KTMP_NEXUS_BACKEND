// services/chat-service/src/routes/message.routes.ts
// Migrate từ src/routes/message.routes.ts
...
/**
 * GET /summary - Lấy tóm tắt (last message, unread count) cho list chats
 */
...
/**
 * GET /:chatId - Lấy tin nhắn của chat
 */
...
messageRoutes.post('/:chatId', asyncHandler(async (req: Request, res: Response) => {
  ...
  const message = await messageService.sendMessage(chatId as string, userId, {
    content,
    type,
    replyToId,
    fileName,
  ...
  res.status(201).json({
    ...
    message: {
      ...
      file: message.fileName
        ? { name: message.fileName, size: message.fileSize, type: message.fileType }
        : null,
      reactions: [],
      isMe: true,
    },
...
/**
 * DELETE /:messageId - Xóa tin nhắn (cho mình)
 */
...
/**
 * DELETE /:messageId/recall - Thu hồi tin nhắn
 */
...
messageRoutes.post('/:messageId/react', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!emoji) {
    return res.status(400).json({ success: false, message: 'Vui lòng chọn emoji!' });
  }
...
/**
 * PUT /:messageId/pin - Ghim/bỏ ghim tin nhắn
 */
messageRoutes.put('/:messageId/pin', asyncHandler(async (req: Request, res: Response) => {
  ...
  res.json({
    success: true,
    message: result.pin ? 'Đã ghim tin nhắn!' : 'Đã bỏ ghim tin nhắn!',
    ...result,
  });
}));

/**
 * GET /:chatId/pinned - Lấy tin nhắn đã ghim
 */
...
/**
 * GET /:chatId/search - Tìm kiếm tin nhắn
 */
...
/**
 * GET /:chatId/media - Lấy media files
 */
